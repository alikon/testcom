...
Cypress.Commands.add('removeOrphanedUpdateSite', (packageName) => {
  ...
  const findOrphans = `
    ...
    LEFT JOIN #__update_sites_extensions usx ON usx.update_site_id = us.update_site_id
...
describe('Package Upgrade Test for alikon/testcom (Latest Release -> PR Candidate, mocked update)', () => {
  ...
  it('1. uninstalls any pre-existing pkg_alikonweb package (cascades to subextensions)', () => {
  ...
  it('2. installs latest stable package release from GitHub', () => {
    cy.request({
      url: 'https://api.github.com/repos/alikon/testcom/releases/latest',
      ...
      // (pkg_alikonweb-x.y.z.zip) invece del trattino usato in precedenza
      // (pkg-alikonweb...). Accettiamo entrambe le varianti.
      const zipAsset = assets.find(
        (asset) => /^pkg[-_]alikonweb.*\.zip$/i.test(asset.name)
      );
...
<updates>
  ...
  it('9. verifies the package updated and MagicLogin kept its configuration', () => {
    cy.visit('administrator/index.php?option=com_plugins&view=plugins');
    cy.searchForItem(PLUGIN_NAME);
